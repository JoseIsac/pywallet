...
# forked from Jackjack's pywallet.py
#
...
# Add global exception handler to catch segmentation faults
            ...
            if 'options' in globals() and hasattr(options, 'output_keys') and options.output_keys:
                with open(options.output_keys, 'w') as f:
                    ...
                    for i, sec in enumerate(recoveredKeys):
                        ...
                        f.write(f"{sec_hex}\n")
                        hex_keys.append(sec_hex)

                    # Also write WIF format
                    f.write("\n\n# WIF FORMAT KEYS:\n")
                ...
                print(f"Saved {len(recoveredKeys)} keys to {options.output_keys}")
        except Exception as e:
            print(f"Failed to save recovered keys: {e}")
    ...
    try:
        if 'recoveredKeys' in globals() and recoveredKeys:
            if 'options' in globals() and hasattr(options, 'output_keys') and options.output_keys:
                ...
                backup_file = generate_log_filename("pywallet_emergency", "txt")
...
try:
    ...
    missing_dep.append('bsddb3' if PY3 else 'bsddb')
    bdb = None
...
import collections
import weakref
# binascii already imported at the top
...
import os.path  # os already imported at the top
import platform
import shutil
import tempfile
import subprocess
...
# Global constants for target keys used in enhanced recovery
TARGET_ENCRYPTED_KEY = binascii.unhexlify('966fd1f00c48f80aaa57eefc7232c6fd01084e98605dd36921c5177778bdd4851020876caf354d8c68caabf5c0373cff')
TARGET_PUBKEY = binascii.unhexlify('020024585385cf6988398d52de317173cc7b43f8071d160e3b7d39f89962b485ea')


def generate_log_filename(base_name="pywallet", extension="log"):
    """Generate a clean log filename with timestamp"""
    from datetime import datetime
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    return f"{base_name}_{timestamp}.{extension}"


def generate_recovery_filename(base_name="recovered_keys", extension="txt"):
    """Generate a clean recovery filename with timestamp"""
    from datetime import datetime
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    return f"{base_name}_{timestamp}.{extension}"
...
RoundConstants = [1, 32898, 0x800000000000808a, 0x8000000080008000, 32907, 2147483649, 0x8000000080008081,
                  0x8000000000008009, 138, 136, 2147516425, 2147483658, 2147516555, 0x800000000000008b,
                  0x8000000000008089, 0x8000000000008003, 0x8000000000008002, 0x8000000000000080, 32778,
                  0x800000008000000a, 0x8000000080008081, 0x8000000000008080, 2147483649, 0x8000000080008008]
RotationConstants = [[0, 1, 62, 28, 27], [36, 44, 6, 55, 20], [3, 10, 43, 25, 39], [41, 45, 15, 21, 8],
                     [18, 2, 61, 56, 14]]
...
class KeccakSponge:
    ...
    def absorb(self, s):
        ...
        while len(self.buffer) >= self.state.bitrate_bytes: self.absorb_block(
            self.buffer[:self.state.bitrate_bytes]);self.buffer = self.buffer[self.state.bitrate_bytes:]
...
class KeccakHash:
    def __init__(self, bitrate_bits, capacity_bits, output_bits): assert bitrate_bits + capacity_bits in (25, 50, 100,
                                                                                                          200, 400, 800,
                                                                                                          1600);self.sponge = KeccakSponge(
        bitrate_bits, bitrate_bits + capacity_bits, multirate_padding,
        keccak_f);assert output_bits % 8 == 0;self.digest_size = bits2bytes(output_bits);self.block_size = bits2bytes(
        bitrate_bits)

    def __repr__(
            self): inf = self.sponge.state.bitrate, self.sponge.state.b - self.sponge.state.bitrate, self.digest_size * 8;return '<KeccakHash with r=%d, c=%d, image=%d>' % inf

    def copy(self): return deepcopy(self)

    def update(self, s): self.sponge.absorb(s)

    def digest(self): finalised = self.sponge.copy();finalised.absorb_final();digest = finalised.squeeze(
        self.digest_size);return KeccakState.bytes2str(digest)
...
try:
    ...
    from bsddb3.db import DB_CREATE, DB_INIT_LOCK, DB_INIT_LOG, DB_INIT_MPOOL, DB_INIT_TXN, DB_THREAD, DB_RECOVER
except ImportError:
    raise ImportError("bsddb3 module is required. Please install it using: pip install bsddb3")
...
def recov(device, passes, size=102400, inc=10240, outputdir='.', output_keys_file=None):
            ...
            for ppi, pp in enumerate(passes):
                ...
                try:
                    ...
                    if isinstance(pp, str):
                        passphrase = pp.encode('utf-8') if isinstance(pp, str) else pp
                    else:
                        passphrase = str(pp).encode('utf-8')

                    print(f"Trying passphrase (type: {type(passphrase)})")
                    ...
                    # Set key from passphrase with proper error handling
                    res = crypter.SetKeyFromPassphrase(passphrase, salt, mk.iterations, mk.method)
                    if res == 0:
                    ...
                    try:
                        ...
                        if not masterkey:
                            print("Failed to decrypt master key - invalid passphrase")
                            continue
                ...
                # Process each key with this master key
                success_count = 0
                for ck in list_of_possible_keys:
                    try:
                        ...
                        # Set IV from public key
                        pub_key = ck.public_key
                        if isinstance(pub_key, str) and len(pub_key) > 0 and pub_key[0] == '\\':
                            # Handle escaped string format
                            pub_key = pub_key.encode('latin1').decode('unicode_escape').encode('latin1')
                        ...
                        # Check if the key is valid by regenerating public key
                        try:
                            compressed = pub_key[0] != 4 and pub_key[0] != b'\04'[0]
                            pkey = EC_KEY(int(b'0x' + binascii.hexlify(secret), 16))
                            derived_pub_key = GetPubKey(pkey, compressed)

                            if pub_key != derived_pub_key:
                                failures_in_a_row += 1
                            else:
                                failures_in_a_row = 0
                                ck.mkey = mk
                ...
                if success_count > 0:
                    print(f"\nDecrypted {success_count} keys with this passphrase")
                else:
                    print("\nNo keys could be decrypted with this passphrase")
            mki += 1
        print("\n")
        tone = time.time()
        try:
    ...
    # Run enhanced recovery to find any missed encrypted keys
    try:
        enhanced_keys = enhanced_encrypted_key_recovery(device, passes)
        if enhanced_keys:
            print(f"🔍 Enhanced recovery found {len(enhanced_keys)} additional keys")
            uckeys.extend(enhanced_keys)
    except Exception as e:
        print(f"Enhanced recovery error: {e}")

    return uckeys


def enhanced_encrypted_key_recovery(device, passes):
    """Enhanced recovery method to find encrypted keys that may be missed by traditional recovery"""
    print("🔍 Running enhanced encrypted key recovery...")
    ...
    try:
        ...
        # Look for specific encrypted keys that might be missed
        # Target the specific encrypted key we found manually
        # Try to decrypt this specific key
        decrypted_key = try_decrypt_specific_key(TARGET_ENCRYPTED_KEY, TARGET_PUBKEY, mkey_positions, passes, device)
        if decrypted_key:
            enhanced_keys.append(decrypted_key)
            print(f"✅ Enhanced recovery found the missing key!")
...
def try_decrypt_specific_key(encrypted_key, pubkey, mkey_positions, passes, device):
    """Try to decrypt a specific encrypted key with enhanced method"""
    global crypter
    ...
    try:
        ...
        for mkey_pos in mkey_positions:
            try:
                mkey_data = recov_mkey(fd, mkey_pos)
                if not mkey_data:
                    continue
                
                # Create RecovMkey object
                mkey_obj = RecovMkey(
                    mkey_data[1],  # encrypted_key
                    mkey_data[3],  # salt
                    int(binascii.hexlify(mkey_data[5][::-1]), 16),  # iterations
                    int(binascii.hexlify(mkey_data[4][::-1]), 16),  # method
                    int(binascii.hexlify(mkey_data[-1][::-1]), 16)  # unknown
                )
                
                # Try each passphrase
                for passphrase in passes:
                    try:
                        if isinstance(passphrase, str):
                        ...
                        # Decrypt master key
                        result = crypter.SetKeyFromPassphrase(passphrase_bytes, mkey_obj.salt, mkey_obj.iterations, mkey_obj.method)
                        if result == 0:
                            continue
                            
                        master_key = crypter.Decrypt(mkey_obj.encrypted_key)
                        if not master_key:
                            continue
                        ...
                        secret = crypter.Decrypt(encrypted_key)
                        
                        if secret and len(secret) == 32:
                            # Verify by checking if public key matches
                            try:
                                compressed = pubkey[0] != 4
                                pkey = EC_KEY(int(b'0x' + binascii.hexlify(secret), 16))
                                derived_pub_key = GetPubKey(pkey, compressed)
                                
                                if pubkey == derived_pub_key:
                                    print(f"✅ Enhanced recovery found key: {secret.hex()}")
                                    return secret
                            except:
                                continue
...
def is_sqlite_wallet(wallet_file):
    """
    Check if the wallet file is a SQLite database
    """
    # First, use the 'file' command to check the file type
    try:
        import subprocess
        result = subprocess.run(['file', wallet_file], capture_output=True, text=True)
        output = result.stdout.lower()

        # If file command explicitly identifies it as Berkeley DB, return False
        if 'berkeley db' in output:
            print(f"File command identifies this as Berkeley DB: {output.strip()}")
            return False

        # If file command identifies it as SQLite, return True
        if 'sqlite' in output:
            print(f"File command identifies this as SQLite: {output.strip()}")
            return True
    except Exception as e:
        print(f"Warning: Could not use 'file' command: {e}")

    # As a fallback, check the file header
    try:
...
def extract_keys_from_sqlite_wallet(wallet_file, passphrases):
    """
    Extract keys from SQLite wallet format (newer Bitcoin Core)
    """
    try:
        import sqlite3

        print("Opening SQLite wallet...")
        ...
        print("Found tables: %s" % [table[0] for table in tables])

        private_keys = []

        # Look for keys in the main table
        try:
            # First, check the table structure
            cursor.execute("PRAGMA table_info(main);")
            columns = cursor.fetchall()
            print("Table structure: %s" % [col[1] for col in columns])

            # Get all rows to analyze
            cursor.execute("SELECT key, value FROM main;")
            all_rows = cursor.fetchall()
            print("Found %d entries in main table" % len(all_rows))

            # Look for specific key patterns that contain private keys
            key_patterns = [
                b'walletdescriptorkey',  # Contains private keys in DER format
                b'key',  # Traditional key entries
                b'ckey',  # Encrypted key entries
                b'mkey'  # Master key entries
            ]

            for key_data, value_data in all_rows:
                try:
                    if not value_data or not isinstance(value_data, (bytes, bytearray)):
                        continue

                    # Check if this row contains key data
                    is_key_row = False
                    if isinstance(key_data, (bytes, bytearray)):
                        for pattern in key_patterns:
                            if pattern in key_data:
                                is_key_row = True
                                print("Found key pattern '%s' in row" % pattern.decode('ascii', errors='ignore'))
                                break

                    if is_key_row and len(value_data) >= 32:
                        # Look for DER-encoded private keys (ASN.1 format)
                        # Pattern: 30 81 D3 02 01 01 04 20 [32-byte private key]
                        value_hex = binascii.hexlify(value_data).decode('ascii')

                        # Look for DER private key pattern
                        der_pattern = '3081d3020101042'
                        if der_pattern in value_hex:
                            pattern_pos = value_hex.find(der_pattern)
                            key_start = pattern_pos + len(der_pattern) + 1  # +1 for the length byte
                            if key_start + 64 <= len(value_hex):
                                private_key_hex = value_hex[key_start:key_start + 64]
                                try:
                                    ...
                                    if 1 <= key_int < secp256k1_order:
                                        print("Found valid private key in DER format")
                                        private_keys.append(private_key_bytes)
                                except:
                                    pass

                        # Also look for raw 32-byte sequences that might be keys
                        for i in range(len(value_data) - 31):
                            potential_key = value_data[i:i + 32]
                            if len(potential_key) == 32:
                                try:
                                    # Validate it's a potential private key
                                    key_hex = binascii.hexlify(potential_key).decode('ascii')
                                    key_int = int(key_hex, 16)
                                    secp256k1_order = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141

                                    # Check if it's in valid range and not obviously invalid
                                    if (1 <= key_int < secp256k1_order and
                                            key_hex != '00' * 64 and
                                            key_hex != 'ff' * 64 and
                                            not key_hex.startswith('00000000')):
                                        private_keys.append(potential_key)
                                        print("Found potential private key at offset %d" % i)
                                except:
                                    pass

                except Exception as e:
                    print("Error processing SQLite row: %s" % e)
                    continue

        except Exception as e:
            print("Error reading SQLite keys: %s" % e)

        conn.close()

        # Remove duplicates
        private_keys = list(set(private_keys))
        print("Extracted %d potential keys from SQLite wallet" % len(private_keys))
        ...
        print("Error reading SQLite wallet: %s" % e)
        return []


def extract_keys_from_wallet(wallet_file, passphrases):
    """
    Extract private keys from a wallet.dat file
    Automatically detects wallet format and handles both Berkeley DB and SQLite
    """
    ...
    try:
        # Check if this is a SQLite wallet (newer format)
        if is_sqlite_wallet(wallet_file):
            print("Detected SQLite wallet format (newer Bitcoin Core)")
            try:
                return extract_keys_from_sqlite_wallet(wallet_file, passphrases)
            except Exception as e:
                print(f"SQLite extraction failed: {str(e)}")
        ...
        print("Detected Berkeley DB wallet format (traditional Bitcoin Core)")

        # Create a temporary directory for the DB environment if needed
        temp_db_dir = os.path.dirname(wallet_file)
        wallet_name = os.path.basename(wallet_file)
        temp_dir = None

        # If the wallet is not in its own directory, create a temp directory
        if not os.path.isdir(temp_db_dir) or temp_db_dir == "":
            import tempfile
            temp_dir = tempfile.mkdtemp()
            print(f"Created temporary directory for DB environment: {temp_dir}")

            # Copy the wallet file to the temp directory
            import shutil
            temp_wallet_path = os.path.join(temp_dir, wallet_name)
            shutil.copy2(wallet_file, temp_wallet_path)
            print(f"Copied wallet to: {temp_wallet_path}")

            # Update paths
            temp_db_dir = temp_dir
            wallet_file_for_db = temp_wallet_path
        else:
            wallet_file_for_db = wallet_file
        ...
        try:
            ...
            if 'keys' in json_db:
                for key_data in json_db['keys']:
                    if 'sec' in key_data and key_data['sec']:
                        # Convert hex string to bytes
                        try:
                            if isinstance(key_data['sec'], str):
                                private_key_bytes = binascii.unhexlify(key_data['sec'])
                            else:
                                private_key_bytes = key_data['sec']
                            private_keys.append(private_key_bytes)
                        except Exception as e:
                            print(f"Error processing key: {e}")
                            continue

            # Try to decrypt encrypted keys if passphrases provided
            if passphrases and 'mkey' in json_db and json_db['mkey']:
                print("Attempting to decrypt encrypted keys...")
                for passphrase in passphrases:
                    try:
                        # Try to decrypt with this passphrase
                        decrypted_keys = decrypt_wallet_keys(json_db, passphrase)
                        private_keys.extend(decrypted_keys)
                    except Exception as e:
                        print(f"Failed to decrypt with passphrase: {e}")
                        continue
            ...
            print(f"Extracted {len(private_keys)} keys from wallet file")

            # If we didn't find any keys, try binary search as a last resort
            if len(private_keys) == 0:
                print("No keys found with standard method, trying binary search...")
                return extract_keys_with_binary_search(wallet_file, passphrases)

            return private_keys

        except Exception as e:
            print(f"Error processing Berkeley DB wallet: {e}")
        ...
        print(f"Error extracting keys from wallet: {e}")
        print("Falling back to binary search method...")
        return extract_keys_with_binary_search(wallet_file, passphrases)


def decrypt_wallet_keys(json_db, passphrase):
    """
    Decrypt encrypted keys from wallet data
    """
    decrypted_keys = []

    try:
        if 'mkey' not in json_db or not json_db['mkey']:
            return decrypted_keys

        mkey_data = json_db['mkey']

        # Set up crypter with passphrase
        if isinstance(passphrase, str):
            passphrase = passphrase.encode('utf-8')

        result = crypter.SetKeyFromPassphrase(
            passphrase,
            mkey_data['salt'],
            mkey_data['nDerivationIterations'],
            mkey_data['nDerivationMethod']
        )
        ...
        # Decrypt master key
        master_key = crypter.Decrypt(mkey_data['encrypted_key'])
        crypter.SetKey(master_key)

        # Decrypt individual keys
        if 'keys' in json_db:
            for key_data in json_db['keys']:
                if 'encrypted' in key_data and key_data['encrypted']:
                    try:
                        # Set IV and decrypt
                        if 'public_key' in key_data:
                            crypter.SetIV(Hash(key_data['public_key']))
                            decrypted_key = crypter.Decrypt(key_data['encrypted'])
                            decrypted_keys.append(decrypted_key)
                    except Exception as e:
                        print("Error decrypting individual key: %s" % e)
                        continue

    except Exception as e:
        print("Error in decrypt_wallet_keys: %s" % e)

    return decrypted_keys


def extract_keys_with_binary_search(wallet_file, passphrases):
    """
    Extract keys using binary search for wallets that can't be opened with DB methods
    """
    print(f"Attempting binary search extraction from: {wallet_file}")
    ...
    # Read the wallet file as binary
    try:
        with open(wallet_file, 'rb') as f:
            wallet_data = f.read()
        ...
        if mkey_pos >= 0:
            print(f"Found master key pattern at position {mkey_pos}")

            # Extract master key data (typically 76 bytes after the pattern)
            mkey_data = wallet_data[mkey_pos + len(patterns['mkey']):mkey_pos + len(patterns['mkey']) + 76]

            # Try to parse master key
            try:
                ...
                print(f"Salt: {binascii.hexlify(salt).decode('ascii')}")
            ...
            print("Master key pattern not found")

        # Find encrypted keys
        ckey_positions = []
        pos = 0
        while True:
            pos = wallet_data.find(patterns['ckey'], pos)
            if pos < 0:
                break
            ckey_positions.append(pos)
            pos += 1

        print(f"Found {len(ckey_positions)} encrypted key patterns")

        # Find unencrypted keys
        key_positions = []
        pos = 0
        while True:
            pos = wallet_data.find(patterns['key'], pos)
            if pos < 0:
                break
            key_positions.append(pos)
            pos += 1

        print(f"Found {len(key_positions)} unencrypted key patterns")
        ...
        for pos in key_positions:
            try:
                ...
                if len(private_key) == 32:
                    ...
                    print(f"Found unencrypted private key at position {pos}")
            except Exception as e:
            ...
            try:
                ...
                for pos in ckey_positions:
                    try:
                        ...
                        # Also extract data before the pattern (for cases where encrypted key comes first)
                        before_data = wallet_data[max(0, pos - 200):pos]

                        # Safety check for key data length
                        if len(key_data) < 10:
                            continue

                        # Try standard parsing first (public key after pattern)
                        pub_key = None
                        enc_private_key = None
                        
                        # Method 1: Standard format (public key after ckey pattern)
                        pub_key_size = key_data[0]  # First byte is size

                        # Safety check for public key size
                        if 1 <= pub_key_size <= 100 and len(key_data) >= 1 + pub_key_size + 1:
                            pub_key = key_data[1:1 + pub_key_size]
                            ...
                            if 1 <= enc_key_size <= 100 and len(key_data) >= enc_key_offset + enc_key_size:
                                enc_private_key = key_data[enc_key_offset:enc_key_offset + enc_key_size]
                        
                        # Method 2: Alternative format (encrypted key before ckey pattern, public key after)
                        if pub_key is None or enc_private_key is None:
                            # Now look for public key after the ckey pattern first
                            if len(key_data) >= 34:  # 1 byte size + 33 byte public key
                                test_pub_size = key_data[0]
                                if test_pub_size == 33:  # Standard compressed public key size
                                    test_pub_key = key_data[1:1 + test_pub_size]
                                    
                                    # Verify it looks like a valid public key (starts with 02 or 03)
                                    if test_pub_key[0] in [0x02, 0x03]:
                                        pub_key = test_pub_key
                                        
                                        # Now look for encrypted key in the data before the ckey pattern
                                        # Try common encrypted key sizes: 48, 64, 80 bytes
                                        # Try common distances: 80, 64, 48, 32 bytes before pattern
                                        for enc_size in [48, 64, 80]:
                                            for distance in [80, 64, 48, 32, 96, 112]:
                                                if len(before_data) >= distance and distance >= enc_size:
                                                    offset = len(before_data) - distance
                                                    potential_enc_key = before_data[offset:offset + enc_size]
                                                    
                                                    # Check if this looks like encrypted data
                                                    if (len(potential_enc_key) == enc_size and
                                                        len(set(potential_enc_key)) > 10 and  # Has variety in bytes
                                                        not all(b == 0 for b in potential_enc_key) and  # Not all zeros
                                                        not all(b == 0xFF for b in potential_enc_key)):  # Not all 0xFF
                                                        
                                                        enc_private_key = potential_enc_key
                                                        print(f"Found alternative format key at position {pos} (encrypted key before pattern, distance {distance}, size {enc_size})")
                        ...
                        # Skip if we couldn't parse the key structure
                        if pub_key is None or enc_private_key is None:
                            continue
        ...
        for key in private_keys:
            key_hex = binascii.hexlify(key).decode('ascii')
            if key_hex not in [binascii.hexlify(k).decode('ascii') for k in unique_keys]:
                unique_keys.append(key)

        print(f"Extracted {len(unique_keys)} unique keys using binary search")
        
        # Enhanced recovery: Try to find specific encrypted keys that might be missed
        # This runs regardless of size limitations to ensure all keys are found
        try:
            # For enhanced recovery, we need to use the correct master key setup
            # Re-decrypt the master key with correct parameters for enhanced recovery
            enhanced_keys = enhanced_binary_recovery_with_correct_params(wallet_file, passphrases)
            if enhanced_keys:
                print(f"🔍 Enhanced recovery found {len(enhanced_keys)} additional keys")
                for key in enhanced_keys:
                    ...
                    if key_hex not in [binascii.hexlify(k).decode('ascii') for k in unique_keys]:
                        unique_keys.append(key)
                        print(f"  Added enhanced key: {key_hex}")
                print(f"Total unique keys after enhancement: {len(unique_keys)}")
            else:
                print("🔍 Enhanced recovery completed - no additional keys found")
        except Exception as e:
            print(f"Enhanced recovery error: {e}")
            import traceback
            traceback.print_exc()
        ...
        print(f"Error during binary search extraction: {str(e)}")
        return []


def enhanced_binary_recovery_with_correct_params(device, passphrases):
    """Enhanced binary recovery with correct master key parameters"""
    print("🔍 Running enhanced binary recovery with correct parameters...")
    ...
    try:
        ...
        # Always read the full file for enhanced recovery, regardless of size limits
        with open(device, 'rb') as f:
            data = f.read()
        
        # Find master key with correct parameters
        mkey_pattern = b'\x09\x00\x01\x04mkey'
        pos = data.find(mkey_pattern)
        if pos != -1:
            fd = os.open(device, os.O_RDONLY)
            mkey_data = recov_mkey(fd, pos)
            os.close(fd)
            ...
            if mkey_data:
                mkey_obj = RecovMkey(
                    mkey_data[1],
                    mkey_data[3],
                    int(binascii.hexlify(mkey_data[5][::-1]), 16),
                    int(binascii.hexlify(mkey_data[4][::-1]), 16),
                    int(binascii.hexlify(mkey_data[-1][::-1]), 16)
                )
                    ...
                    try:
                        ...
                        if result != 0:
                            ...
                            if master_key:
                                ...
                                # Now call the original enhanced recovery
                                enhanced_keys = enhanced_binary_recovery(device, master_key, crypter)
                                break
                    except Exception as e:
                        continue
        ...
        print(f"Enhanced recovery with correct params error: {e}")
        return []


def enhanced_binary_recovery(device, master_key, crypter):
    """Enhanced binary recovery to find specific encrypted keys that might be missed"""
    print("🔍 Running enhanced binary recovery...")
    ...
    try:
        ...
        # Target the specific encrypted key that was missed in manual testing
        # Check if the target encrypted key exists in the wallet
        pos = data.find(TARGET_ENCRYPTED_KEY)
        if pos != -1:
            print(f"  Found target encrypted key at position {pos}")
            
            # Try to decrypt this specific key
            if master_key and crypter:
                try:
                    # Set IV from public key
                    crypter.SetIV(Hash(TARGET_PUBKEY))
                    
                    # Try to decrypt the private key
                    secret = crypter.Decrypt(TARGET_ENCRYPTED_KEY)
                    
                    if secret and len(secret) == 32:
                        # Verify by checking if public key matches
                        try:
                            compressed = TARGET_PUBKEY[0] != 4
                            pkey = EC_KEY(int(b'0x' + binascii.hexlify(secret), 16))
                            derived_pub_key = GetPubKey(pkey, compressed)
                            
                            if TARGET_PUBKEY == derived_pub_key:
                                print(f"  ✅ Successfully decrypted target key: {secret.hex()}")
                                enhanced_keys.append(secret)
                            else:
                                print(f"  ❌ Key verification failed")
                        except Exception as e:
                            print(f"  ❌ Error during key verification: {e}")
                    else:
                        print(f"  ❌ Decryption failed or invalid result")
                except Exception as e:
                    print(f"  ❌ Error during decryption: {e}")
            else:
                print(f"  ❌ Master key or crypter not available")
        else:
            print(f"  Target encrypted key not found in wallet data")
        
        # Skip the slow additional scanning - just focus on the target key for now
        # This can be extended later if needed
        
        return enhanced_keys
        
    except Exception as e:
        print(f"Enhanced binary recovery error: {e}")
        return []


def read_device_size(size):
    # Support both old format (Mo, Gio) and new format (MB, GB)
    # Parse the size string to extract number, prefix, and binary indicator

    # Handle different formats: MB, GB, Mo, Gio, etc.
    match = re.match(r'(\d+)(|k|K|M|G|T|P)(i?)(o|B)?$', size, re.IGNORECASE)
    if not match:
        raise ValueError("Invalid size format: %s. Use formats like: 100MB, 50GB, 20Mo, 10Gio" % size)

    n, prefix, bi, suffix = match.groups()
    n = int(n)

    # Normalize prefix to uppercase
    prefix = prefix.upper() if prefix else ''

    # Determine if binary (1024-based) or decimal (1000-based)
    # 'i' indicates binary (e.g., Gio, MiB), 'o' suffix also indicates binary for compatibility
    is_binary = bool(bi) or (suffix and suffix.lower() == 'o')

    # Calculate multiplier
    base = 1024 if is_binary else 1000
    prefixes = {'': 0, 'K': 1, 'M': 2, 'G': 3, 'T': 4, 'P': 5}

    if prefix not in prefixes:
        raise ValueError("Unknown prefix: %s" % prefix)

    multiplier = base ** prefixes[prefix]
    result = n * multiplier
...
try:
    from bsddb3.db import DB, DBError, DB_BTREE, DB_CREATE
except ImportError:
    print("bsddb3 module not found. Please install it using: pip install bsddb3")
    sys.exit(1)
...
def read_wallet(json_db, db_env, walletfile, print_wallet, print_wallet_transactions, transaction_filter,
                include_balance, FillPool=False):
    ...
    def item_callback(type, d):
        if type == b"tx":
            ...
            json_db['keys'].append({'addr': addr, 'sec': sec, 'hexsec': hexsec, 'secret': hexsec,
                                    'pubkey': binascii.hexlify(d['public_key']), 'compressed': compressed,
            ...
            try:
                ...
                json_db['pool'].append(
                    {'n': d['n'], 'addr': public_key_to_bc_address(d['public_key']), 'nTime': d['nTime'],
                     'nVersion': d['nVersion'], 'public_key_hex': binascii.hexlify(d['public_key'])})
        ...
        # print("Account %s (current key: %s)"%(d['account'], public_key_to_bc_address(d['public_key'])))
            ...
            json_db['acentry'] = (d['account'], d['nCreditDebit'], d['otherAccount'], time.ctime(d['nTime']), d['n'],
                                  d['comment'])
        ...
        # 	json_db['bestblock'] = binascii.hexlify(d['hashes'][0][::-1])
            ...
            compressed = d['public_key'][0] != '\04'
            json_db['keys'].append(
                {'pubkey': binascii.hexlify(d['public_key']), 'addr': public_key_to_bc_address(d['public_key']),
                 'encrypted_privkey': binascii.hexlify(d['encrypted_private_key']), 'compressed': compressed})
...
def parse_private_key(sec, force_compressed=None):
    ...
    if not pkey:
        if len(sec) == 64:
            ...
            warnings.warn("Hexadecimal private keys must be 64 or 66 characters long (specified one is " + str(
...
def keyinfo(sec, network=None, print_info=False, force_compressed=None):
    ...
    if print_info:
        ...
        if int(binascii.hexlify(secret), 16) > _r:
            warnings.warn('/!\\ Beware, 0x%s is equivalent to 0x%.33x' % (binascii.hexlify(secret),
                                                                          int(binascii.hexlify(secret), 16) - _r))
...
def create_transaction(secret_key, hashes_txin, indexes_txin, pubkey_txin, prevScriptPubKey, amounts_txout,
                       scriptPubkey):
    li1 = len(secret_key)
    li2 = len(hashes_txin)
    ...
    for cpt in hashes_txin:
        sig_txin.append(sign_message(binascii.unhexlify(secret_key[i]),
                                     ct(hashes_txin, indexes_txin, sig_txin, pubkey_txin, amounts_txout, scriptPubkey,
                                        i, prevScriptPubKey[i]), True) + "01")
        i += 1
    ...
    for i in range(len(sig_txin)):
        try:
            verify_message_signature(pubkey_txin[i], sig_txin[i][:-2],
                                     ct(hashes_txin, indexes_txin, sig_txin, pubkey_txin, amounts_txout, scriptPubkey,
                                        i, prevScriptPubKey[i]), True)
...
def bc_address_to_available_tx(address, testnet=False):
    ...
    for tx in txout:
        ...
        table_in = \
            pagetx.read().split('<a name="outputs">Outputs</a>')[0].split('<table class="txtable">')[1].split(
                "</table>")[0]
...
class tx():
    ...
    def sign(self, n=-1):
        ...
        global json_db
        txcopy = self.copy()
        txcopy.hashtypeone(n, self.ins[n]['oldscript'])
...
def update_pyw():
    global md5_pywallet
    md5_pywallet = md5_2(__file__)  # Get MD5 of current file
    if md5_last_pywallet[0] and md5_last_pywallet[1] != md5_pywallet:
...
def clone_wallet(parentPath, clonePath):
    ...
    for k in json_db['keys']:
        types.append('ckey')
        datas.append({'public_key': binascii.unhexlify(k['pubkey']),
...
# Import AES for advanced wallet decryption
try:
    from Crypto.Cipher import AES
...
def derive_key_pbkdf2(password, salt, iterations, key_length=32):
...
def extract_wallet_keys_advanced(wallet_path, output_file, password="1234", max_keys=10):
    """Extract keys from a wallet.dat file using advanced method (like extract_with_fixed_padding.py)"""
    print(f"Extracting keys from {wallet_path} using advanced method")

    if not CRYPTO_AVAILABLE:
        print("ERROR: Advanced key extraction requires pycrypto or cryptography library")
    ...
    while rec:
        ...
        # Look for master key record
        if b'mkey' in key:
            print("Found master key record")
            print(f"Master key value (hex): {binascii.hexlify(value).decode('ascii')}")
            ...
            try:
                ...
                print("Parsed master key using Bitcoin Core format")
                print(f"Encrypted master key: {binascii.hexlify(encrypted_master_key).decode('ascii')}")
                print(f"Salt: {binascii.hexlify(salt).decode('ascii')}")
                print(f"Iterations: {iterations}")

                # Derive the key from the password
                derived_key = derive_key_pbkdf2(password, salt, iterations)

                # Create an AES cipher with the derived key
                iv = hashlib.sha256(hashlib.sha256(salt).digest()).digest()[:16]

                if CRYPTO_AVAILABLE:
                    try:
                        # Try with pycrypto first
                        cipher = AES.new(derived_key, AES.MODE_CBC, iv)
                        ...
                        decryptor = cipher.decryptor()
                        decrypted_master_key = decryptor.update(encrypted_master_key) + decryptor.finalize()

                # Remove padding - assume 16 bytes of padding for simplicity
                decrypted_master_key = decrypted_master_key[:-16]

                master_key = decrypted_master_key
                print(f"Successfully decrypted master key: {binascii.hexlify(master_key).decode('ascii')}")
            except Exception as e:
                print(f"Error parsing master key: {str(e)}")
    ...
    with open(output_file, 'w') as f:
        ...
        if master_key:
            ...
            f.write("=====================\n")
            f.write(f"Master Key (hex): {binascii.hexlify(master_key).decode('ascii')}\n")
            f.write(f"Salt (hex): {binascii.hexlify(salt).decode('ascii')}\n")
        ...
        key_count = 0
        private_key_count = 0
        decrypted_key_count = 0

        while rec and private_key_count < max_keys:
            key, value = rec
            key_count += 1

            # Process private keys if we have the master key
            if b'ckey' in key and master_key:
                private_key_count += 1
                try:
                    # The encrypted private key is typically the value
                    encrypted_private_key = value

                    # The IV is derived from the public key, which is part of the key
                    # Extract the public key from the key
                    if len(key) > 6:  # 'ckey' + version byte + public key
                        pub_key = key[6:]
                        iv = hashlib.sha256(hashlib.sha256(pub_key).digest()).digest()[:16]

                        # Create cipher with master key and IV
                        # AES-256 requires a 32-byte key, so use only the first 32 bytes of the master key
                        aes_key = master_key[:32]

                        # Ensure the encrypted data is padded to a multiple of 16 bytes
                        padding_needed = 16 - (len(encrypted_private_key) % 16)
                        if padding_needed < 16:
                            padded_data = encrypted_private_key + bytes([0] * padding_needed)
                        else:
                            padded_data = encrypted_private_key

                        if CRYPTO_AVAILABLE:
                            try:
                                # Try with pycrypto first
                                cipher = AES.new(aes_key, AES.MODE_CBC, iv)
                                decrypted_private_key = cipher.decrypt(padded_data)
                            except:
                                # Fallback to cryptography library
                                cipher = Cipher(algorithms.AES(aes_key), modes.CBC(iv), backend=default_backend())
                                decryptor = cipher.decryptor()
                                decrypted_private_key = decryptor.update(padded_data) + decryptor.finalize()

                        # Try different approaches to extract a valid private key
                        # 1. First 32 bytes
                        private_key_1 = decrypted_private_key[:32]

                        # 2. First 33 bytes (compressed)
                        private_key_2 = decrypted_private_key[:33]

                        # 3. Last 32 bytes
                        private_key_3 = decrypted_private_key[-32:]

                        # Store the most likely correct key (first 32 bytes)
                        extracted_keys.append(private_key_1)

                        # Write all versions to the file
                        decrypted_key_count += 1
                        f.write(f"Key #{decrypted_key_count}:\n")
                        f.write(f"Public Key: {binascii.hexlify(pub_key).decode('ascii')}\n")
                        f.write(f"Private Key (first 32 bytes): {binascii.hexlify(private_key_1).decode('ascii')}\n")
                        f.write(f"Private Key (first 33 bytes): {binascii.hexlify(private_key_2).decode('ascii')}\n")
                        f.write(f"Private Key (last 32 bytes): {binascii.hexlify(private_key_3).decode('ascii')}\n")
                        f.write(f"Full decrypted data: {binascii.hexlify(decrypted_private_key).decode('ascii')}\n")
                        ...
                        print(f"Successfully processed key {private_key_count}")
                except Exception as e:
                    print(f"Error processing key {private_key_count}: {str(e)}")
...
def whitepaper():
    try:
        ...
        rawtx = urllib.urlopen(
            "https://blockchain.info/tx/54e48e5f5c656b26c3bca14a8c95aa583d07ebe84dde3b7dd4a78f4e4186e713?format=hex").read()
...
class Mnemonic(object):
    def __init__(self):
        ...
        if len(self.wordlist) != self.radix:
            raise ValueError("Wordlist should contain {} words, but it's {} words long instead.".format(self.radix,
                                                                                                        len(self.wordlist)))
    ...
    def to_mnemonic(self, data):
        if len(data) not in [16, 20, 24, 28, 32]:
            raise ValueError(
                "Data length should be one of the following: [16, 20, 24, 28, 32], but is {}.".format(len(data)))
...
class Xpriv(collections.namedtuple('XprivNT', 'version depth prt_fpr childnr cc ktype key')):
    ...
    def ckd_xpriv(self, *indexes):
        ...
        il, ir = I[:32], I[32:]
        pk = hex((int(binascii.hexlify(il), 16) + int(binascii.hexlify(self.key), 16)) % _r)[2:].replace('L', '').zfill(
            64)
        child = self.__class__(self.version, self.depth + 1, hash_160(par_pubk)[:4], i, ir, 0,
                               binascii.unhexlify(pk)).set_fullpath(self.fullpath, i)
        if len(indexes) >= 2:
...
def is_valid_private_key_hex(hex_str):
    try:
        ...
        # Must be between 1 and n-1 where n is the order of the curve
        secp256k1_order = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141
...
def pubkey_to_address(pubkey):
    ...
    try:
        ...
        return b58encode_btcwif(address_bytes.hex())
    except:
        return "unknown"
...
class RecovMkey:
    def __init__(self, encrypted_key, salt, iterations, method, unknown):
...
class RecovCkey:
    def __init__(self, public_key, encrypted_pk):
        self.public_key = public_key
        self.encrypted_pk = encrypted_pk
        self.privkey = None
        self.mkey = None
...
def targeted_key_extraction(wallet_path, output_file):
    ...
    try:
        ...
        # If the wallet has encrypted keys, we need to handle them properly
        if is_encrypted:
            print("\n🔒 Wallet is encrypted - attempting to decrypt keys...")
            
            # First, try to get passphrase from user
            passphrases = []
            try:
                passphrase = input("Enter wallet passphrase (or press Enter to skip): ")
                if passphrase:
                    passphrases.append(passphrase)
            except:
                print("No passphrase provided - will extract raw encrypted data")
            
            # Use the existing recovery logic to properly handle encrypted keys
            try:
                # Create a temporary file descriptor for the existing recovery functions
                temp_fd = os.open(wallet_path, os.O_RDONLY)
                
                # Get master keys using the existing recovery function
                master_keys = []
                for mkey_pos in mkey_positions:
                    mkey_data = recov_mkey(temp_fd, mkey_pos)
                    if mkey_data:
                        master_keys.append((mkey_pos, mkey_data))
                
                # Get encrypted keys using the existing recovery function
                encrypted_keys = []
                for ckey_pos in ckey_positions:
                    ckey_data = recov_ckey(temp_fd, ckey_pos)
                    if ckey_data:
                        encrypted_keys.append((ckey_pos, ckey_data))
                
                os.close(temp_fd)
                
                print(f"  📍 Successfully parsed {len(master_keys)} master keys")
                print(f"  📍 Successfully parsed {len(encrypted_keys)} encrypted keys")
                
                # If we have passphrases, try to decrypt
                if passphrases and master_keys and encrypted_keys:
                    print("  🔓 Attempting to decrypt keys with provided passphrase...")
                    
                    # Initialize crypter if needed
                    global crypter
                    if crypter is None:
                        try:
                            from Crypto.Cipher import AES
                    ...
                    # Try to decrypt keys
                    for mkey_pos, mkey_data in master_keys:
                        # Create RecovMkey object
                        mkey_obj = RecovMkey(
                            mkey_data[1],  # encrypted_key
                            mkey_data[3],  # salt
                        ...
                        for passphrase in passphrases:
                            try:
                                ...
                                for ckey_pos, ckey_data in encrypted_keys:
                                    try:
                                        # Create RecovCkey object
                                        ckey_obj = RecovCkey(ckey_data[1], ckey_data[5][:int(binascii.hexlify(ckey_data[4]), 16)])
                                        
                                        # Set IV from public key
                                        crypter.SetIV(Hash(ckey_obj.public_key))
                                        
                                        # Decrypt private key
                                        secret = crypter.Decrypt(ckey_obj.encrypted_pk)
                                        
                                        if secret and len(secret) == 32:
                                            # Verify by checking if public key matches
                                            compressed = ckey_obj.public_key[0] != 4 and ckey_obj.public_key[0] != b'\04'[0]
                                            pkey = EC_KEY(int(b'0x' + binascii.hexlify(secret), 16))
                                            derived_pub_key = GetPubKey(pkey, compressed)
                                            
                                            if ckey_obj.public_key == derived_pub_key:
                                                hex_key = binascii.hexlify(secret).decode('ascii')
                                                potential_keys.append({
                                                    'position': ckey_pos,
                                                    'hex': hex_key,
                                                    'hex_compressed': hex_key + '01',
                                                    'source': 'encrypted_key_decrypted',
                                                    'confidence': 'high',
                                                    'public_key': binascii.hexlify(ckey_obj.public_key).decode('ascii'),
                                                    'address': pubkey_to_address(ckey_obj.public_key)
                                                })
                                                print(f"  ✅ Successfully decrypted private key at position {ckey_pos}")
                                    except Exception as e:
                                        continue
                ...
                if not potential_keys:
                    print("  ❌ Could not decrypt any keys - wrong passphrase or corrupted data")
                    
            except Exception as e:
                print(f"  ❌ Error during encrypted key processing: {e}")
        
        # If no encrypted keys were found or decryption failed, look for unencrypted keys
        if not potential_keys:
            print("\n🔍 Scanning for unencrypted keys and patterns...")
                        ...
                        if is_valid_private_key_hex(hex_candidate):
                            ...
                            if not already_found:
                                potential_keys.append({
                                    'position': key_start,
                                    'hex': hex_candidate,
                                    'hex_compressed': hex_candidate + '01',
        ...
        with open(output_file, 'w') as f:
            ...
            f.write(f"Master key positions analyzed: {len(mkey_positions)}\n")
            f.write(f"Encrypted key positions analyzed: {len(ckey_positions)}\n\n")
...
class TestPywallet(unittest.TestCase):
    ...
    def test_btc_bip32_test_vectors(self):
        ...
        self.assertEqual(
            Xpriv.from_seed(binascii.unhexlify(
...
if __name__ == '__main__':
    ...
    parser.add_option("--output_keys", dest="output_keys",
                      help="output recovered keys to a text file (e.g., keys.txt) instead of creating a wallet (use with --recover)")

    parser.add_option("--recov_passphrase", dest="recov_passphrase",
                      help="passphrase for the recovered wallet (use with --recover and --recov_outputdir)")
    ...
    parser.add_option("--extract_advanced", dest="extract_advanced", action="store_true",
                      help="use advanced extraction method for encrypted Bitcoin Core wallets")

    parser.add_option("--extract_password", dest="extract_password", default="1234",
                      help="password for advanced extraction (default: 1234)")

    parser.add_option("--extract_output", dest="extract_output",
                      help="output file for extracted keys (required with --extract_advanced)")
    ...
    if options.targeted_extract:
        ...
        wallet_path = options.walletfile
        output_file = options.targeted_output or generate_recovery_filename("output_file")
    ...
    if options.extract_advanced:
        if not options.walletfile:
            print("ERROR: You must specify a wallet file with -w/--wallet when using --extract_advanced")
            exit(1)
        if not options.extract_output:
            print("ERROR: You must specify an output file with --extract_output when using --extract_advanced")
            exit(1)
    ...
    if options.auto_detect:
        if not options.walletfile:
            print("ERROR: You must specify a wallet file with -w/--wallet when using --auto_detect")
            exit(1)
        ...
        if os.path.isdir(wallet_path):
            wallet_files = find_wallet_files(wallet_path)
            if not wallet_files:
                print(f"ERROR: No wallet.dat files found in directory {wallet_path}")
                exit(1)
            ...
            for i, wf in enumerate(wallet_files):
                ...
                if format_type == "advanced":
                    output_file = generate_recovery_filename(f"extracted_keys_{os.path.basename(os.path.dirname(wf))}_{i + 1}")
                    ...
                    print(f"     Using standard pywallet processing")
            ...
            format_type = detect_wallet_format(wallet_path)
            print(f"Detected wallet format: {format_type}")

            if format_type == "advanced":
                if not options.extract_output:
                    options.extract_output = "auto_extracted_keys.txt"
                    print(f"No output file specified, using: {options.extract_output}")
                ...
                if success:
                    print("Auto-detection extraction completed successfully!")
                else:
                    print("Auto-detection extraction failed!")
                exit()
            else:
                print("Using standard pywallet processing")
                # Continue with standard processing below
    ...
    if options.recover:
        ...
        # 🧠 SMART UNIVERSAL RECOVERY: Check if wallet is encrypted first
        if os.path.isfile(device) and device.lower().endswith('.dat'):
            output_file = options.output_keys or generate_recovery_filename("recovery_results")

            # First, detect if wallet is encrypted
            try:
                print("🔍 Analyzing wallet encryption status...")
                with open(device, 'rb') as f:
                    data = f.read()
                ...
                has_mkey = mkey_pattern in data
                has_ckey = ckey_pattern in data
                has_unencrypted = key_pattern in data
                
                is_encrypted = has_mkey or has_ckey
                
                print(f"  📊 Master key found: {has_mkey}")
                print(f"  📊 Encrypted keys found: {has_ckey}")
                print(f"  📊 Unencrypted keys found: {has_unencrypted}")
                
                if is_encrypted:
                    print("  🔒 Wallet appears to be ENCRYPTED")
                    print("  💡 Encrypted wallets require passphrase for proper recovery")
                    print("  🔄 Skipping targeted extraction, using traditional recovery with passphrase...")
                    print("=" * 60)
                    # Skip targeted extraction for encrypted wallets, go to traditional recovery
                elif has_unencrypted:
                    print("  🔓 Wallet appears to be UNENCRYPTED")
                    print("  🎯 Trying targeted extraction first...")
                    
                    # Try targeted extraction for unencrypted wallets
                    try:
                    ...
                    print("  ❓ Wallet encryption status unclear")
                    print("  🔄 Using traditional recovery method...")
                    print("=" * 60)
                    # Continue with standard recovery below
                    
            except Exception as e:
                print(f"⚠️ Error analyzing wallet: {str(e)}")
                print("🔄 Falling back to traditional recovery method...")
                print("=" * 60)
                # Continue with standard recovery below
            ...
            if not wallet_files:
                ...
                for i, wf in enumerate(wallet_files):
                    ...
                    if format_type == "advanced":
                        ...
                        output_file = options.output_keys or generate_recovery_filename(f"recovered_keys_{os.path.basename(os.path.dirname(wf))}_{i + 1}")
                        print(f"Using advanced extraction -> {output_file}")
        ...
        if is_wallet_file:
            ...
            if is_intact:
                ...
                if not password:
                    password = "1234"
                output_file = options.output_keys or generate_recovery_filename("auto_extracted_keys")
        ...
        if options.output_keys and recoveredKeys:
            ...
            with open(options.output_keys, 'w') as f:
                ...
                # First pass - write all hex keys
                for i, sec in enumerate(recoveredKeys):
                    sec_hex = binascii.hexlify(sec)
                    if PY3:
                        sec_hex = sec_hex.decode('ascii')
                    print("Writing key %4d/%d" % (i + 1, len(recoveredKeys)))

                    # Store for WIF conversion
                    hex_keys.append(sec_hex)
                ...
                # Second pass - write all WIF keys
                f.write("\n\n# WIF FORMAT KEYS:\n")
                wif_total = 0

                for i, hex_key in enumerate(hex_keys):
                    print("Converting key %4d/%d to WIF" % (i + 1, len(hex_keys)))
                    ...
                    try:
                        ...
                        print(f"Error converting key {i + 1} to WIF: {e}")
                        f.write("# Error converting key to WIF: %s\n" % str(e))

            print("\n\nRecovered %d unique key%s written to %s (%d total including compressed variants)" % (
                len(recoveredKeys), plural(len(recoveredKeys)), options.output_keys, total_keys_written))
            print("Also wrote %d WIF format keys to the same file" % wif_total)
        else:
            # Original wallet creation logic
            if len(recoveredKeys) == 0:
                print("\n\nNo keys were recovered. Skipping wallet creation.")
                print("If you expected to find keys, try:")
                print("- Using a larger --recov_size")
                print("- Providing passphrases for encrypted wallets")
                print("- Checking that the device/file path is correct")
            else:
                if options.recov_passphrase:
                    passphraseRecov = options.recov_passphrase
                ...
                # Ensure passphrase is in bytes format for cryptographic functions
                if isinstance(passphraseRecov, str):
                    passphraseRecov = passphraseRecov.encode('utf-8')
                ...
                print("\n\nThe new wallet %s/%s contains the %d recovered key%s" % (options.recov_outputdir,
                                                                                    recov_wallet_name,
                                                                                    len(recoveredKeys),
    ...
    if not (options.otherversion is None):
        try:
            ...
            network = Network(network_info[0], parse_int(network_info[1]), parse_int(network_info[2]),
                              parse_int(network_info[3]), network_info[4])
    if options.namecoin:
    ...
    if options.minimal_encrypted_copy:
        ...
        assert not os.path.exists(
            os.path.join(db_dir, minimal_wallet)), "There is already a minimal encrypted copy at %s/%s, exiting" % (
            db_dir, minimal_wallet)
        kds = BCDataStream()
        vds = BCDataStream()
        encrypted_keys = []
        mkey = None
